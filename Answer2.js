...
var $ = function ( selector, context ) {
  ...
  while (currentSelector.length !== 0) {
      ...
      if (elements.length === 0) {
        ...
        for (var i = 0; i < elements.length; i++) {
          ...
          if (elements[i].className.search(re) !== -1) {
            // console.log("classes: " + elements[i].className + "      expecting: " + match[1])
    ...
    if (currentSelector === previousSelector) {
      ...
      throw new Error( "Syntax error – unrecognized expression: " + currentSelector);
    } else {
  ...
  if (remainingSelector.length !== 0) {
    to_keep = [];
    for (var i = 0; i < elements.length; i++) {
      var children = $(remainingSelector, elements[i]);
      to_keep.push.apply(to_keep, children);
    }
    elements = to_keep;
  }
  if (elements.length === 0 ) {
